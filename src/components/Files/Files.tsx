  ...
  useEffect(() => {
    const loadTemplatesAndLink = async () => {
      try {
        ...
        if (!templateId && storeTemplates.items.length > 0) {
          templateId = pickTemplateIdForDevice(storeTemplates.items);
        }

        if (templateId) {
          setCreateLink(`/app/editor/default?template=${templateId}`);
        }
      } catch (e) {
  ...
  const handleCreateInvoice = async () => {
    ...
    try {
      ...
      if (templateId) {
        ...
        router.push(`/app/editor/default?template=${templateId}`, 'forward', 'push');
      } else {
