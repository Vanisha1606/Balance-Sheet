...
import { toLatinInput, filterWesternNumeric, sanitizeCellInput, containsDevanagari } from '../../../utils/english-input';
...
const CellEditModal: React.FC<CellEditModalProps> = ({
    ...
    const openedCoordRef = useRef<string | null>(null);

    if (isOpen) {
        (window as unknown as { __cellEditModalActive?: boolean }).__cellEditModalActive = true;
    }

    const attachSocialCalcPassThru = useCallback(() => {
        ...
        const sc = (window as unknown as {
            SocialCalc?: {
                CmdGotFocus?: (obj: HTMLElement) => void;
                Keyboard?: { passThru?: unknown };
            };
        }).SocialCalc;
    ...
    // Keep SocialCalc from stealing keys (it clears passThru on space via KeyboardSetFocus)
    useLayoutEffect(() => {
        if (!isOpen) {
            (window as unknown as { __cellEditModalActive?: boolean }).__cellEditModalActive = false;
            openedCoordRef.current = null;
            const sc = (window as unknown as { SocialCalc?: { KeyboardFocus?: () => void } }).SocialCalc;
            sc?.KeyboardFocus?.();
            return;
        }

        (window as unknown as { __cellEditModalActive?: boolean }).__cellEditModalActive = true;
        attachSocialCalcPassThru();
        const keepPassThru = window.setInterval(attachSocialCalcPassThru, 200);

        return () => {
            window.clearInterval(keepPassThru);
            (window as unknown as { __cellEditModalActive?: boolean }).__cellEditModalActive = false;
            openedCoordRef.current = null;
        };
    }, [isOpen, attachSocialCalcPassThru]);

    // Set initial cell text once per open — never re-bind from React state while typing
    useLayoutEffect(() => {
        if (!isOpen || !cellData) {
            return;
        }
        if (openedCoordRef.current === cellData.coord) {
            return;
        }
        openedCoordRef.current = cellData.coord;

        const raw = cellData.text || '';
        const initial =
            cellData.inputType === 'number'
                ? filterWesternNumeric(toLatinInput(raw))
                : toLatinInput(raw);

        textBufferRef.current = initial;
        setHasText(initial.length > 0);

        if (inputRef.current) {
            inputRef.current.value = initial;
            hardenEnglishInput(inputRef.current);
        }
    }, [isOpen, cellData?.coord, cellData?.text, cellData?.inputType]);

    // Initialize values once when modal opens (do not reset while user is typing)
    useEffect(() => {
        if (!isOpen || !cellData) {
            return;
        }
    ...
    }, [isOpen, cellData?.coord, attachSocialCalcPassThru]);
    ...
    const handleApply = () => {
        ...
        const raw = inputRef.current?.value ?? textBufferRef.current;
        const sanitizedValue = sanitizeCellInput(raw, !!isNumericInput);

        if (import.meta.env.DEV) console.log('Applying change:', sanitizedValue, 'to', cellData.coord);
        ...
        try {
            ...
            if (AppGeneral.updateCellValueAndFormat) {
                AppGeneral.updateCellValueAndFormat(cellData.coord, sanitizedValue, formatting);
            } else {
            ...
            // Emergency fallback
            if (cellData.okfn) cellData.okfn(sanitizedValue);
        }
    ...
    const isNumericInput = cellData?.inputType === 'number';
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const needsSanitize =
            ...
            /[०-९٠-٩]/.test(raw) ||
            (isNumericInput && /[^0-9.\-]/.test(raw));

        if (needsSanitize) {
            const cursor = el.selectionStart ?? raw.length;
            raw = sanitizeCellInput(raw, !!isNumericInput);
            el.value = raw;
            const pos = Math.min(cursor, raw.length);
            el.setSelectionRange(pos, pos);
        }

        textBufferRef.current = raw;
        setHasText(raw.length > 0);
    };

    const hardenEnglishInput = (el: HTMLInputElement) => {
        el.lang = 'en-US';
    ...
    return (
        ...
        >
            <div className="cell-edit-modal-content">
                ...
                {/* Input Section */}
                <div className="cell-edit-input-section" lang="en-US">
                    ...
                    <input
                        ...
                        name="english-cell-value"
                        pattern={isNumericInput ? '[0-9]*' : undefined}
                        defaultValue=""
                        onChange={handleInputChange}
                        onInput={handleInputChange}
                        onFocus={attachSocialCalcPassThru}
                        onPaste={(e) => {
                            e.preventDefault();
                            ...
                            const sanitized = sanitizeCellInput(merged, !!isNumericInput);
                            el.value = sanitized;
                            textBufferRef.current = sanitized;
                            el.setSelectionRange(sanitized.length, sanitized.length);
                            setHasText(sanitized.length > 0);
                        }}
                        onKeyDown={(e) => {
                            attachSocialCalcPassThru();
                            e.stopPropagation();
                        ...
                        className="cell-edit-input english-only-input"
                    />
                    </form>
                    {hasText && (
                        <div
