...
/** Vacation Planner stores SocialCalc EditableCells directly in appMapping */
export const isSocialCalcEditableCellsMapping = (
    appMapping: unknown
): appMapping is { allow: boolean; cells: Record<string, boolean>; constraints: Record<string, string[]> } => {
    if (!appMapping || typeof appMapping !== 'object') return false;
    const map = appMapping as Record<string, unknown>;
    return map.allow !== undefined && !!map.cells && !!map.constraints;
};
