...
export function updateCellValueAndFormat(coord, val, formatting) {
  ...
  if (val !== null && val !== undefined) {
    const normalized = String(val)
      ...
      .replace(/[\u0900-\u097F]/g, "");
    const sheetCell = editor.context.sheetobj.cells[coord];
    const sheetname = editor.workingvalues?.currentsheet;
    const col = coord.replace(/[0-9]/g, "");
    const forceNumeric = sheetname === "list1" && (col === "D" || col === "E");
    const cleaned = forceNumeric ? normalized.replace(/[^0-9.\-]/g, "") : normalized;
    var numVal = parseFloat(cleaned);
    if (forceNumeric) {
      if (cleaned === "") {
        cmds.push("set " + coord + " empty");
      } else if (!isNaN(numVal) && isFinite(numVal)) {
        cmds.push("set " + coord + " value n " + numVal);
      }
      // Budget columns: never save text values
    } else if (
      !isNaN(numVal) &&
      isFinite(cleaned) &&
      cleaned.toString().trim() === numVal.toString()
    ) {
      cmds.push("set " + coord + " value n " + numVal);
    } else {
      var strVal;
      if (SocialCalc.encodeForSave) {
        strVal = SocialCalc.encodeForSave(cleaned.toString());
      } else {
        strVal = cleaned.toString().replace(/\\/g, "\\b").replace(/:/g, "\\c").replace(/\n/g, "\\n");
      }
