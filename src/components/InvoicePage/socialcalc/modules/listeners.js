...
import { enhancedShowPrompt, isToggleCheckboxCell } from "./prompts.js";

/** True when the React cell-edit modal is open and must own the keyboard */
function isCellEditActive() {
  return (
    (typeof window !== "undefined" && window.__cellEditModalActive) ||
    isIonModalOpen() ||
    !!document.getElementById("vp-cell-edit-overlay")
  );
}
function isIonModalOpen() {
...
export function setupMouseListener() {
  ...
  SocialCalc.ProcessEditorMouseDown = function (e) {
    ...
    const isCheckboxCell = isToggleCheckboxCell(editor, result.coord);

    // Toggle Cell Callback (packing-list checkboxes)
    if (isCheckboxCell && SocialCalc.Callbacks && SocialCalc.Callbacks.ToggleCell) {
  ...
  const originalEditorOpenCellEdit = SocialCalc.EditorOpenCellEdit;
  const originalEditorProcessKey = SocialCalc.EditorProcessKey;
  const originalKeyboardSetFocus = SocialCalc.KeyboardSetFocus;
  const originalKeyboardFocus = SocialCalc.KeyboardFocus;

  SocialCalc.KeyboardSetFocus = function (editor) {
    if (isCellEditActive()) return;
    return originalKeyboardSetFocus.apply(this, arguments);
  };

  SocialCalc.KeyboardFocus = function () {
    if (isCellEditActive()) return;
    return originalKeyboardFocus.apply(this, arguments);
  };

  SocialCalc.EditorOpenCellEdit = function (editor) {
    if (isCellEditActive()) return true;
    if (!editor?.ecell) return true;
    if (SocialCalc.Callbacks?.IsCellEditable && !SocialCalc.Callbacks.IsCellEditable(editor)) {
      return true;
    }
    if (isToggleCheckboxCell(editor, editor.ecell.coord)) {
      return true;
    }
    enhancedShowPrompt(editor.ecell.coord);
    return false;
  ...
  SocialCalc.EditorProcessKey = function (editor, ch, e) {
    ...
    if (
      ...
      if (SocialCalc.Callbacks?.IsCellEditable && !SocialCalc.Callbacks.IsCellEditable(editor)) {
        return originalEditorProcessKey.apply(this, arguments);
      }
      if (isToggleCheckboxCell(editor, editor.ecell.coord)) {
        return originalEditorProcessKey.apply(this, arguments);
      }
      enhancedShowPrompt(editor.ecell.coord, ch);
      return false;
    }
    return originalEditorProcessKey.apply(this, arguments);
  };

  const isInputBlocked = () => isCellEditActive();
  ...
  return function cleanup() {
    ...
    SocialCalc.EditorOpenCellEdit = originalEditorOpenCellEdit;
    SocialCalc.EditorProcessKey = originalEditorProcessKey;
    SocialCalc.KeyboardSetFocus = originalKeyboardSetFocus;
    SocialCalc.KeyboardFocus = originalKeyboardFocus;
