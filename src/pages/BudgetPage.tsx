...
import {
  ...
  isSocialCalcEditableCellsMapping,
...
const BudgetPage: React.FC = () => {
  ...
  useEffect(() => {
    const handleCellEditRequest = (event: CustomEvent) => {
      if ((window as unknown as { __cellEditModalActive?: boolean }).__cellEditModalActive) {
        return;
      }
      const { coord, text, inputType, okfn, cleanup } = event.detail;
      setCellEditData({ coord, text, inputType, okfn, cleanup });
  ...
  const initializeApp = async () => {
    ...
    try {
      ...
      if (isNewInvoiceMode && templateIdFromUrl) {
        ...
        // Vacation Planner templates store EditableCells in appMapping; HM templates use sheet mappings
        const editableCells = isSocialCalcEditableCellsMapping(templateData.appMapping)
          ? templateData.appMapping
          : (templateData.appMapping && Object.keys(templateData.appMapping).length > 0)
            ? {
        ...
        <IonToolbar color="primary" className="invoice-toolbar" style={{ paddingTop: `max(env(safe-area-inset-top, 0px), 24px)`, minHeight: '56px' }}>
          ...
          >
            <div style={{ display: "flex", alignItems: "center" }}>
              <span>{fileName === "invoice" ? "New File" : fileName}</span>
            </div>
          </IonButtons>
          ...
          >
            ...
            >
              <IonButton
                fill="solid"
                size="default"
                onClick={() => history.push("/app/dashboard/home")}
